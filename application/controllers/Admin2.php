...
class Admin extends CI_Controller
{
    ...
    public function client_creation_form()
    {
        ...
        $this->db->select('*')->from('employee_itgs')->where('role', 'CM')->or_where('role', 'TL');
    ...
    public function job_dashboard()
    {
        ...
        if ($_SESSION['role'] == "Internal Auditor") {
            $data['jobs'] = $this->db->query('select fund_request_activity.*,case_request.client_reference,case_request.reference_code,scope_of_work.scope_name, employee_itgs.employee_name, sub_scope.name as sub_category from fund_request_activity left JOIN case_request on(case_request.id=fund_request_activity.case_id) left join scope_of_work on (scope_of_work.id=fund_request_activity.activity_id) left join employee_itgs on employee_itgs.id = fund_request_activity.employe_id left join sub_scope on sub_scope.id = fund_request_activity.sub_scope_id order by fund_request_activity.id desc')->result_array();
        } else if ($_SESSION['role'] == "Manager Finance") {
            $data['jobs'] = $this->db->query('select fund_request_activity.*,case_request.client_reference,case_request.reference_code,scope_of_work.scope_name, count(fund_approve.id) as fid, employee_itgs.employee_name, sub_scope.name as sub_category from fund_request_activity left JOIN case_request on(case_request.id=fund_request_activity.case_id) left join scope_of_work on (scope_of_work.id=fund_request_activity.activity_id) left join fund_approve on fund_approve.fund_id = fund_request_activity.id left join employee_itgs on employee_itgs.id = fund_request_activity.employe_id left join employee_itgs on employee_itgs.id = fund_request_activity.employe_id where fund_request_activity.is_approved = 1 GROUP BY fund_request_activity.id')->result_array();
    ...
    {
        $data['cvs'] = $this->admin_model->select_where('recruitment', array(
            'status' => '1'
        ));
    ...
    {
        $this->admin_model->delete_data('employee_itgs', array(
            'id' => $id
        ));
    ...
    {
        //$data['employees']=$this->db->get('employee_itgs')->result_array();
        $data['employees'] = $this->admin_model->select_where('employee_itgs', array(
            'role' => 'vendor'
        ));
        $this->load->view('admin2/header', $data);
    ...
    public function insert_report()
    {
        ...
        if ($this->db->insert('case_report', $data)) {
            ...
            $case_detail    = $this->admin_model->select_where('case_request', array(
                'id' => $this->input->post('case_id_report')
            ));
            $emails['data'] = array(
            ...
            // $employee = $this->admin_model->get_client_email($_POST['case_id_report']);
            // $this->sendmail($employee[0]['login_name'],$employee[0]['email'],'Visiting Case Submit Report','Clieck Here To Detail <a href="'.base_url().'admin/view_report_submission/'.$_POST['case_id_report'].'">View Request</a>');
            $this->db->update('case_request', array(
                'case_status' => 5
            ), array(
                'id' => $_POST['case_id_report']
            ));
            $client       = $this->admin_model->select_where('case_request', array(
                'id' => $_POST['case_id_report']
            ));
    ...
    public function step2($case_id)
    {
        ...
        $data['subjects']       = $this->db->get_where("subject_case", array(
            'case_id' => $case_id
        ))->result_array();
        $data['services']       = $this->db->query("select scope_of_work.scope_name,assign_client_services.* from scope_of_work inner join assign_client_services on(assign_client_services.scope_id=scope_of_work.id) where assign_client_services.client_id='" . $_SESSION['client_id'] . "'")->result_array();
    ...
    public function fund_request_update()
    {
        ...
        $this->db->update('fund_request_activity', array(
    ...
    {
        $where         = $_SESSION['client_id'];
        $data['cases'] = $this->admin_model->select_where('case_request', array(
            'client_id' => $where,
            'case_status' => 0
        ));
    ...
    {
        $data['team_lead'] = $this->db->get_where('employee_itgs', array(
            'role' => 'TM',
    ...
    {
        $data['profile'] = $this->db->get_where('login', array(
    ...
    public function view_reports()
    {
        ...
        $data['total_customer'] = $this->db->get_where('clients', array(
            'is_client' => 1
        ))->num_rows();
    ...
    public function create_lead()
    {
        ...
        $this->is_login();
        $data['clients'] = $this->db->get_where('clients', array(
    ...
    public function interview_calender()
    {
        ...
        $data['records'] = $this->admin_model->select_where('interviews', array(
            'interviewer' => $id
        ));
        //echo '<pre>';print_r($data);die;
        $this->load->view('admin2/header');
    ...
    public function create_emp($id = null)
    {
        ...
        $data['ceo'] = $this->admin_model->select_where('login', array(
            'role' => 'Ceo'
        ));
        $this->load->view('admin2/header');
    ...
    {
        $data['employee'] = $this->admin_model->select_where('employee_itgs', array(
            'id' => $this->session->userdata('id')
        ));
    ...
    public function leave_application()
    {
        ...
        $status           = $this->admin_model->select_where('employee_detail', array(
            'employee_code' => $data['employee'][0]['employee_id']
        ));
        if (!empty($satus)) {
    ...
    public function insert_request()
    {
        ...
        if (!$this->upload->do_upload('file')) {
            ...
            if ($this->admin_model->insert_request($data)) {
                ...
                $array_json = array(
                    'status' => false,
                    'message' => 'Error in Inserting Lead'
                );
    ...
    public function get_notification()
    {
        ...
        $this->db->select('*')->from('notifications')->where('user_id', $id)->where('user_type', $type)->where('view', '0')->order_by('id', 'desc');
    ...
    {
        $this->admin_model->update('notifications', array(
            'view' => '1'
        ), array(
            'id' => $id
        ));
    ...
    {
        $data = $this->db->get_where("sales_create_meeting", array(
    ...
    public function assign_meeting()
    {
        ...
        $get_userID = $this->db->get_where("sales_create_meeting", array(
            'meetingID' => $_POST['meetingID']
        ))->row_array();
    ...
    public function view_customer_lead($clientID)
    {
        ...
        $data['customer'] = $this->db->get_where('clients', array(
            'clientID' => $clientID
        ))->row_array();
    ...
    {
        $this->admin_model->update_data('resignation', array(
            'status' => '1'
        ), array(
            'id' => $id
        ));
        $employee     = $this->admin_model->select_where_row('employee_itgs', array(
...
window.location.href='" . base_url() . "admin/view_lead/'
    ...
    public function update_payment()
    {
        ...
        if ($this->db->update('lead', $data, array(
            'leadID' => $_POST['leadID']
        ))) {
            echo "<script>
...
window.location.href='" . base_url() . "admin/add_payment/'
    ...
    public function view_customer()
    {
        ...
        $data['customers'] = $this->db->get_where('client', array(
            'status' => 1,
    ...
    public function view_customer_report()
    {
        ...
        $data['customers'] = $this->db->get_where('clients', array(
            'is_client' => 1
        ))->result_array();
        $this->load->view('admin/header');
    ...
    public function check_client()
    {
        ...
        $company      = $this->db->get_where('clients', array(
    ...
    public function employee_login()
    {
        ...
        if ($client) {
            ...
            $date        = date('Y-m-d');
            $resignation = $this->admin_model->select_where_row('resignation', array(
                'employee_id' => $employee_id,
                'requested_date <' => $date,
                'status' => 1
            ));
    ...
    public function update_case($status = null)
    {
        ...
        $this->db->update('case_request', $data_case, array(
            'id' => $case_id
        ));
        $due_date = $_POST['due_date'];
        $this->db->select('id')->from('subject_case')->where('case_id', $case_id);
        ...
        for ($i = 0; $i < $count_subject; $i++) {
            ...
            if (!empty($subject_id)) {
                ...
                $this->db->update('subject_case', $data_subject, array(
                    'id' => $subject_id
                ));
                $sub_activity = $this->admin_model->select_where('subject_activities', array(
                    'subject_id' => $subject_id
                ));
                for ($j = 0; $j < $count_activity; $j++) {
                    ...
                    if ($subject_activity[$j] && !empty($subject_activity[$j])) {
                        ...
                        $this->db->update('subject_activities', $data_activity, array(
                            'id' => $subject_activity[$j]
                        ));
                    } else {
                        $data = array(
                            'activity_id' => $activity_id[$j],
        ...
        if (sizeof($sub_current) >= 1) {
            for ($i = 0; $i < sizeof($sub_current); $i++) {
                $this->admin_model->delete_data('subject_case', array(
                    'id' => $sub_current[$i]['id']
                ));
                $this->admin_model->delete_data('subject_activities', array(
                    'subject_id' => $sub_current[$i]['id']
                ));
            }
        }
        $emails['data'] = array(
    ...
    public function load_client_chat()
    {
        ...
        $this->admin_model->update_data('client_chat', array(
            'is_view' => 1
        ), array(
            'case_id' => $case_id,
            'type' => $type
...
?> clearfix ">
							<div class=chat-avatar>
								<img src="<?php
            echo base_url();
?>admin_assets/assets/dist/img/avatar4.png" alt=male>
								<i><?php
            echo $key['chat_date_time'];
?></i>
							</div>
...
?>
				                 <a href="<?php
                echo base_url();
?>uploads/chat_attachment/<?php
                echo $key['attachement'];
?>" target="_blank"><?php
                echo $key['attachement'];
?></a>
...
?>
    ...
    public function insert_team()
    {
        ...
        if (!empty($_POST['subject_id'])) {
            ...
            for ($i = 0; $i < sizeof($subject); $i++) {
                ...
                $tl = $this->admin_model->select_where('case_team', array(
                    'case_id' => $_POST['case_id'],
                ...
                $this->db->update("subject_case", array(
                    'is_assigned' => 1
                ), array(
                    'id' => $_POST['subject_id'][$i]
                ));
            }
            $update_status = $this->db->update('case_request', array(
                'case_status' => 7
            ), array(
                'id' => $_POST['case_id']
            ));
    ...
    {
        $data['case']     = $this->db->get_where('case_request', array(
            'id' => $case_id
        ))->row_array();
        $data['subjects'] = $this->db->get_where('subject_case', array(
            'case_id' => $case_id
...
?>
                                             <tr>
                                                <td><?php
            echo $teams['subject_name'];
?> </td>
                                                <td><?php
            echo $teams['team_lead'];
?> </td>
                                                <td><?php
            echo $key['team_member'];
?> </td>
                                                <?php
            if ($teams['team_lead_id'] == $_SESSION['id']) {
?>
                                                <td>
                                                    <a class="btn btn-primary" data-toggle="modal" href='#assign_activity' onclick="assign_activity_user(<?php
                echo $key['team_member_id'];
?>,<?php
                echo $subject_id;
?>)">Assign Activity</a>
                                                     </td>
...
?>
    ...
    public function get_subject_activities()
    {
        ...
        $select_assigned_activities = $this->db->get_where('assign_activity_to_user', array(
            'subject_id' => $subject_id
        ))->result_array();
...
?>
	       ...
	       <input type="checkbox" disabled="" name="activity_id[]" value="<?php
                echo $activity['activity_id'];
?>"> <?php
                echo $activity['scope_name'];
?> - Due Date : <?php
                echo $activity['due_date'];
?>
...
?>
	                 <div><input type="checkbox" name="activity_id[]" value="<?php
                echo $activity['activity_id'];
?>"> <?php
                echo $activity['scope_name'];
?> - Due Date : <?php
...
?>
    ...
    public function assign_team_member()
    {
        ...
        for ($i = 0; $i < count($activity_id); $i++) {
            ...
            $links      = array();
            for ($p = 0; $p < sizeof($file_links); $p++) {
                $links[] = '/uploads/activity_files/' . $file_links[$p]['file'];
            }
            //print_r($links);die;
    ...
    public function assign_vendor_request()
    {
        ...
        if ($this->db->insert('assign_vendor_request', $data)) {
            ...
            if ($last) {
                $last_row = $this->admin_model->select_where_row('assign_vendor_request', array(
                    'case_id' => $this->input->post('vendor_case_id'),
                    'subject_id' => $this->input->post('vendor_subject_id'),
                    'activity_id' => $this->input->post('vendor_activity_id'),
                    'vendor_id' => $last
                ));
                $this->admin_model->delete_data('assign_vendor_request', array(
                    'id' => $last_row['id']
                ));
                $last_row['old_id'] = $last_row['id'];
                unset($last_row['id']);
                $this->db->insert('cancel_vendor_request', $last_row);
                $this->admin_model->delete_data('case_fund_request', array(
                    'case_id' => $this->input->post('vendor_case_id'),
                    'subject_id' => $this->input->post('vendor_subject_id'),
                    'activity_id' => $this->input->post('vendor_activity_id'),
                    'vendor_id' => $last
    ...
    public function insert_fund_request()
    {
        ...
        if ($this->db->insert('fund_request_activity', $data)) {
            $employee = $this->admin_model->select_where('employee_itgs', array(
                'id' => $_SESSION['id']
            ));
            $employee = $employee[0];
    ...
    public function add_recruitment()
    {
        ...
        if (!empty($_FILES)) {
            ...
            if ($this->upload->do_upload('file')) {
                ...
                $data += array(
                    'file' => '/uploads/cv/' . $this->upload->data('file_name')
                );
            } else {
                $error = array(
    ...
    public function update_recruitment()
    {
        ...
        for ($i = 0; $i < sizeof($ids); $i++) {
            $this->admin_model->update_data('recruitment', array(
                'status' => '1'
            ), array(
                'id' => $ids[$i]
            ));
    ...
    {
        $this->admin_model->delete_data('interviews', array(
            'id' => $id
        ));
    ...
    public function add_emp()
    {
        ...
        if (!empty($_FILES)) {
            ...
            if ($this->upload->do_upload('profile_photo')) {
                $data += array(
                    'image' => '/uploads/profile_photo/' . $this->upload->data('file_name')
                );
            } else {
                $error = array(
                    'error' => $this->upload->display_errors()
    ...
    {
        $data['recruitment']                = $this->admin_model->select_where('recruitment', array(
            'id' => $id
        ));
        $data['recruitment_experiences']    = $this->admin_model->select_where('recruitment_experiences', array(
            'recruitment_id' => $id
        ));
        $data['recruitment_qualifications'] = $this->admin_model->select_where('recruitment_qualifications', array(
            'recruitment_id' => $id
        ));
    ...
    public function employee_detail($id)
    {
        ...
        $data['employee_educations']         = $this->admin_model->select_where('employee_educations', array(
            'employee_id' => $id
        ));
        $data['employee_employment_history'] = $this->admin_model->select_where('employee_employment_history', array(
            'employee_id' => $id
        ));
        $data['employee_previous_address']   = $this->admin_model->select_where('employee_previous_address', array(
            'employee_id' => $id
        ));
        $data['employee_reference']          = $this->admin_model->select_where('employee_reference', array(
            'employee_id' => $id
        ));
        $this->load->view('admin2/header');
    ...
    {
        $data['appointment'] = $this->admin_model->select_where('appointments', array(
            'id' => $id
        ));
    ...
    {
        $id = $this->admin_model->update('employee', array(
            'active' => '1'
        ), array(
            'id' => $id
        ));
    ...
    public function past_employee()
    {
        ...
        $data['records']   = $this->admin_model->view_employee();
        $data['employees'] = $this->admin_model->select_where('employee', array(
            'active' => '1'
        ));
        $this->load->view('admin2/header');
    ...
    public function edit_emp()
    {
        ...
        $id   = $this->admin_model->update("employee", $data, array(
            'id' => $employee_id
        ));
        $data = array(
            'current_address' => $this->input->post('current_address'),
        ...
        $this->admin_model->update("employee_address", $data, array(
            'employee_id' => $employee_id
        ));
        $data = array(
            'employee_code' => $this->input->post('employee_code'),
        ...
        $this->admin_model->update("employee_detail", $data, array(
            'employee_id' => $employee_id
        ));
        ...
        $school_year_completed = $this->input->post('school_year_completed[]');
        $school_dgree          = $this->input->post('school_dgree[]');
        $education             = $this->admin_model->select_where('employee_educations', array(
            'employee_id' => $employee_id
        ));
        $edu_delete            = array();
        for ($i = 0; $i < sizeof($school_type); $i++) {
            ...
            if ($key >= 0) {
                $edu_delete[] = $key;
                $this->admin_model->update("employee_educations", $data, array(
                    'employee_id' => $employee_id,
                    'id' => $education[$key]['id']
                ));
            } else {
        ...
        if (sizeof($education) >= 1) {
            for ($i = 0; $i < sizeof($education); $i++) {
                $this->admin_model->delete_data('employee_educations', array(
                    'id' => $education[$i]['id']
                ));
            }
        }
        ...
        $nature_of_employment    = $this->input->post('nature_of_employment[]');
        $reason_for_leaving      = $this->input->post('reason_for_leaving[]');
        $history                 = $this->admin_model->select_where('employee_employment_history', array(
            'employee_id' => $employee_id
        ));
        $his_delete              = array();
        for ($i = 0; $i < sizeof($name_of_employer); $i++) {
            ...
            if ($key >= 0) {
                $his_delete[] = $key;
                $this->admin_model->update("employee_employment_history", $data, array(
                    'employee_id' => $employee_id,
                    'id' => $history[$key]['id']
                ));
            } else {
        ...
        if (sizeof($history) >= 1) {
            for ($i = 0; $i < sizeof($history); $i++) {
                $this->admin_model->delete_data('employee_employment_history', array(
                    'id' => $history[$i]['id']
                ));
            }
        }
        ...
        $this->admin_model->update("employee_other", $data, array(
            'employee_id' => $employee_id
        ));
        ...
        $privious_country             = $this->input->post('privious_country[]');
        $privious_citizen             = $this->input->post('privious_citizen[]');
        $previous                     = $this->admin_model->select_where('employee_previous_address', array(
            'employee_id' => $employee_id
        ));
        $pre_delete                   = array();
        for ($i = 0; $i < sizeof($previous_address); $i++) {
            ...
            if ($key >= 0) {
                $pre_delete[] = $key;
                $this->admin_model->update("employee_previous_address", $data, array(
                    'employee_id' => $employee_id,
                    'id' => $previous[$key]['id']
                ));
            } else {
        ...
        if (sizeof($previous) >= 1) {
            for ($i = 0; $i < sizeof($previous); $i++) {
                $this->admin_model->delete_data('employee_previous_address', array(
                    'id' => $previous[$i]['id']
                ));
            }
        }
        ...
        $ref_email       = $this->input->post('ref_email[]');
        $ref_address     = $this->input->post('ref_address[]');
        $reference       = $this->admin_model->select_where('employee_reference', array(
            'employee_id' => $employee_id
        ));
        $ref_delete      = array();
        for ($i = 0; $i < sizeof($ref_name); $i++) {
            ...
            if ($key >= 0) {
                $ref_delete[] = $key;
                $this->admin_model->update("employee_reference", $data, array(
                    'employee_id' => $employee_id,
                    'id' => $reference[$key]['id']
                ));
            } else {
    ...
    public function add_visiting_card()
    {
        ...
        $admins = $this->admin_model->select_where('login', array(
            'role' => 'Admin'
        ));
    ...
    {
        $this->admin_model->delete_data("departments", array(
            'id' => $id
        ));
        redirect('admin/departments');
    }
    ...
    public function update_department()
    {
        ...
        $this->admin_model->update("departments", array(
            'name' => $name,
            'parent_id' => $parent_id
        ), array(
            'id' => $id
    ...
    {
        $data['departments']      = $this->admin_model->get_data('departments');
        $data['main_departments'] = $this->admin_model->select_where('departments', array(
            'parent_id' => '0'
        ));
        $this->load->view('admin2/header');
    ...
    {
        $departments       = $this->admin_model->select_where_row('departments', array(
            'id' => $id
        ));
        $data['employees'] = $this->admin_model->select_where('employee_itgs', array(
    ...
    {
        $data['employee']  = $this->admin_model->select_where_row('employee_itgs', array(
            'id' => $id
        ));
        $data['employees'] = $this->admin_model->select_where('employee_itgs', array(
            'report_to' => $id
    ...
    public function view_report_case($id)
    {
        ...
        for ($i = 0; $i < sizeof($data['education']); $i++) {
            $children = $this->admin_model->select_where('education_check_key', array(
                'education_id' => $data['education'][$i]['id']
            ));
            if (!empty($children)) {
        ...
        for ($i = 0; $i < sizeof($data['past']); $i++) {
            $children = $this->admin_model->select_where('past_employment_key', array(
                'past_id' => $data['past'][$i]['id']
            ));
            if (!empty($children)) {
        ...
        $data['team_lead']  = $this->admin_model->select_where('case_team', array(
            'case_id' => $id
        ));
        $data['tl_report']  = $this->admin_model->select_where('tl_report', array(
            'case_id' => $id
        ));
        $data['case_id']    = $id;
    ...
    public function edit_team($case_id)
    {
        ...
        //$data['subjects']=$this->db->get_where("subject_case",array('case_id'=>$case_id))->result_array();
        /*$data['members']=$this->db->query("SELECT * FROM `employee_itgs` WHERE `role` = 'TL' OR `role` = 'CM'")->result_array();
        $data['members1']=$this->db->query("SELECT * FROM `employee_itgs` WHERE `role` = 'TL' OR `role` = 'CM' OR `role` = 'TM'")->result_array();*/
        $data['members']        = $this->db->query("SELECT * FROM `employee_itgs` WHERE `role` = 'TL'")->result_array();
        $data['members1']       = $this->db->query("SELECT * FROM `employee_itgs` WHERE `role` = 'TM'")->result_array();
    ...
    public function update_team()
    {
        if (!empty($_POST['subject_id'])) {
            ...
            for ($i = 0; $i < sizeof($subject); $i++) {
                ...
                if ($count >= 1) {
                    $this->admin_model->delete_data('case_team_members', array(
                        'subject_id' => $subject[$i],
                        'case_team_id' => $case_team_id
                    ));
                    for ($a = 0; $a < $count; $a++) {
    ...
    public function vendor_assign()
    {
        ...
        if (!empty($_POST['subject_id'])) {
            ...
            for ($i = 0; $i < sizeof($subject); $i++) {
                ...
                $this->admin_model->insert_data("notifications", $notification);
                $activity_id = $this->admin_model->select_where('subject_activities', array(
                    'case_id' => $_POST['case_id'],
                    'subject_id' => $subject[$i]
                ));
    ...
    public function issue_payment()
    {
        ...
        $fund = $this->admin_model->select_where_row('fund_request_activity', array(
            'id' => $_POST['fund_id']
        ));
        if ($fund) {
            $user = $this->admin_model->select_where_row('employee_itgs', array(
                'id' => $fund['employee_id']
            ));
            if ($user) {
                $notification = array(
    ...
    {
        $this->admin_model->update("leave_applications", array(
            'is_approve' => '2'
        ), array(
            'id' => $id
        ));
    ...
    {
        $this->admin_model->update("visiting_card_forms", array(
            'issue' => '1'
        ), array(
            'id' => $id
        ));
    ...
    {
        $this->admin_model->update("loan_applications", array(
            'is_approve' => $update
        ), array(
            'id' => $id
        ));
    ...
    {
        $this->admin_model->update("salary_advance_forms", array(
            'is_approve' => $update
        ), array(
            'id' => $id
        ));
    ...
    {
        $this->admin_model->update("expense_forms", array(
            'is_approve' => $update
        ), array(
            'id' => $id
        ));
    ...
    {
        $this->admin_model->update("complaint_forms", array(
    ...
    {
        $this->admin_model->update("recruitment", array(
            'latter' => '2'
        ), array(
            'id' => $id
        ));
    ...
    public function insert_training()
    {
        ...
        if (!empty($_FILES)) {
            ...
            if ($this->upload->do_upload('training_material')) {
                $data += array(
                    'training_material' => '/uploads/training_material/' . $this->upload->data('file_name')
                );
            } else {
                $error = array(
                    'error' => $this->upload->display_errors()
    ...
    {
        $data['departments']          = $this->admin_model->get_data('departments');
        $data['training']             = $this->admin_model->select_where('training', array(
            'id' => $id
        ));
        $data['training_departments'] = $this->admin_model->select_where('training_department', array(
            'training_id' => $id
        ));
        $this->load->view('admin2/header');
    ...
    public function update_training($value = '')
    {
        ...
        if (!array_key_exists('training_badge_earned', $data)) {
            $data += array(
                'training_badge_earned' => '0'
            );
        }
        if (!array_key_exists('self_enrollment_allowed', $data)) {
            $data += array(
                'self_enrollment_allowed' => '0'
            );
        }
        ...
        $this->admin_model->update_data("training", $data, array(
            'id' => $id
        ));
        $this->admin_model->delete_data('training_department', array(
            'training_id' => $id
        ));
        $apply = $this->input->post('apply');
        for ($i = 0; $i < sizeof($apply); $i++) {
    ...
    public function training_hr($id)
    {
        ...
        $data['id']                   = $this->admin_model->select_where('approve_training', array(
            'training_id' => $id,
            'employee_id' => $_SESSION['id']
        ));
        $this->load->view('admin2/header');
    ...
    public function edit_course($id)
    {
        ...
        $data['records'] = $this->admin_model->get_data('employee_itgs');
        $data['course']  = $this->admin_model->select_where('courses', array(
            'id' => $id
        ));
        $this->load->view('admin2/header');
    ...
    public function update_course()
    {
        ...
        unset($data['id']);
        $id = $this->input->post('id');
        $this->admin_model->update_data("courses", $data, array(
            'id' => $id
        ));
        redirect('admin/view_course');
    }
    ...
    {
        $this->admin_model->update("courses", array(
            'status' => '1'
        ), array(
            'id' => $id
        ));
    ...
    {
        $data = $this->admin_model->select_where('chapter', array(
            'course_id' => $id
        ));
        echo json_encode($data);
    }
    ...
    {
        $data = $this->admin_model->select_where('section', array(
            'chapter_id' => $id
        ));
        echo json_encode($data);
    }
    ...
    public function update_report()
    {
        ...
        for ($i = 0; $i < $count_subject; $i++) {
            ...
            $this->db->update('case_aplicant', $data_case, array(
    ...
    public function insert_customized_report()
    {
        ...
        $this->admin_model->update_data("assign_activity_to_user", array(
            'is_report' => '1',
    ...
    {
        $this->admin_model->delete_data('subject_dummy_attechment', array(
            'id' => $id
        ));
    }
    ...
    {
        $this->admin_model->delete_data('activity_dummy_attechment', array(
            'id' => $id
        ));
    }
    ...
    public function education_verification()
    {
        ...
        if ($id) {
            $this->admin_model->update_data("assign_activity_to_user", array(
                ...
                'id' => $this->input->post('assign_activity_id')
            ));
    ...
    public function insert_report_vendor()
    {
        ...
        $data['file'] = implode(',', $links);
        $this->admin_model->update_data("assign_vendor_request", array(
            'is_report' => '1',
            'report_time' => date('Y-m-d')
        ), array(
            'id' => $this->input->post('assign_vendor_id')
        ));
        $this->admin_model->insert_data("vendor_report", $data);
        $vendor = $this->admin_model->select_where_row('assign_vendor_request', array(
            'id' => $data['assign_vendor_id']
        ));
        if ($vendor) {
            $user = $this->admin_model->select_where_row('assign_activity_to_user', array(
                'case_id' => $vendor['case_id'],
                'activity_id' => $vendor['activity_id']
            ));
            if ($user) {
                $user_detail = $this->admin_model->select_where_row('employee_itgs', array(
                    'id' => $user['member_id']
                ));
                if ($user_detail) {
                    $notification = array(
    ...
    {
        $data['courses'] = $this->admin_model->select_where('courses', array(
            'status' => '0'
        ));
        $this->load->view('admin2/header');
    ...
    {
        $this->db->update('case_fund_request', array(
            'is_approve' => $_GET['status']
        ), array(
            'id' => $_GET['request_id'],
            'case_id' => $_GET['case_id']
        ));
    ...
    public function issue_case_payment()
    {
        ...
        $case    = $this->admin_model->select_where('case_fund_request', array(
            'id' => $_POST['fund_id']
        ));
        $case_id = $case[0]['case_id'];
        $this->db->update('assign_vendor_request', array(
            'is_assigned' => 1
        ), array(
            'case_id' => $case_id
        ));
    ...
    public function edit_employee_admin()
    {
        $data = $_POST;
        unset($data['id']);
        $this->admin_model->update_data("employee_itgs", $data, array(
            'id' => $this->input->post('id')
        ));
    ...
    public function leaves_management()
    {
        ...
        $two        = date('Y-12-31', strtotime($first));
        $start      = date('Y-m-d', strtotime($first));
        $total_days = $this->countDays(date('Y', strtotime($first)), date('m', strtotime($first)), array(
            0,
            6
        ));
        $this->db->select("id as total,employee_id,clock_in,STR_TO_DATE('091559','%h%i%s') as last,'12:00:01' as end")->from('attendance')->where('date >=', $first)->where('date <=', $last)->where('employee_id !=', '')->having('clock_in > last AND clock_in < end');
        $c = $this->db->get_compiled_select();
        $this->db->select("id as total,employee_id,clock_in,'12:00:00' as last")->from('attendance')->where('date >=', $first)->where('date <=', $last)->where('employee_id !=', '')->having('clock_in > last');
        $h = $this->db->get_compiled_select();
        $this->db->select("id as total,employee_id,clock_out,'03:00:01' as last")->from('attendance')->where('date >=', $first)->where('date <=', $last)->where('employee_id !=', '')->having('clock_out < last');
        $s = $this->db->get_compiled_select();
        $this->db->select("count(id) as total, employee_id")->from('attendance')->group_by('employee_id')->where('date >=', $first)->where('date <=', $last)->where('employee_id !=', '');
        $a = $this->db->get_compiled_select();
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($first))->where('l.start_date <=', DATE($last))->where("(l.is_approve = '4' OR l.is_approve = '7')")->where('l.nature_of_leave', 'Half Day')->group_by('e.id');
        $e = $this->db->get_compiled_select();
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($one))->where('l.start_date <=', DATE($two))->where("l.is_approve = '4'")->group_by('e.id');
        $al = $this->db->get_compiled_select();
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($first))->where('l.start_date <=', DATE($last))->where("(l.is_approve = '4' OR l.is_approve = '7')")->where('l.nature_of_leave !=', 'Half Day')->group_by('e.id');
        $ec = $this->db->get_compiled_select();
        $this->db->select('e.employee_name,e.employee_id, concat(' . $total_days . ' - a.total) as leaves, count(c.total) as late, group_concat(DISTINCT(h.total) separator ",") as hafe, group_concat(DISTINCT(s.total) separator ",") as short, ec.days as approved, eh.days as short_approved,sum(ed.sick_leave + ed.casual_leave + ed.annual_leave) as total_leaves,al.days as old_leaves,count(l.id) as ab')->from('employee_itgs e')->join('employee_detail ed', 'e.employee_id = ed.employee_code', 'left')->join('leave_applications l', 'l.employee_id = e.employee_id and l.start_date = "' . $start . '" and l.absent = 1', 'left')->join('(' . $a . ') a', 'a.employee_id = e.employee_id')->join('(' . $c . ') c', 'e.employee_id = c.employee_id')->join('(' . $h . ') h', 'e.employee_id = h.employee_id', 'left')->join('(' . $s . ') s', 'e.employee_id = s.employee_id', 'left')->join('(' . $e . ') eh', 'e.employee_id = eh.employee_id', 'left')->join('(' . $ec . ') ec', 'e.employee_id = ec.employee_id', 'left')->join('(' . $al . ') al', 'e.employee_id = al.employee_id', 'left')->group_by('e.id')->having('ab = 0')->order_by('e.employee_name');
        $data['employees'] = $this->db->get()->result_array();
        $data['first']     = $first;
        $data['last']      = $last;
        //print_r($data);die;
    ...
    public function view_leaves($id = null, $month = null)
    {
        ...
        $employees = $this->admin_model->get_data('employee_itgs');
        for ($i = 0; $i < sizeof($employees); $i++) {
            $this->db->select('sum(ed.sick_leave + ed.casual_leave + ed.annual_leave) as leaves')->from('employee_detail ed')->where('ed.employee_code', $employees[$i]['employee_id']);
    ...
    public function payrole()
    {
        ...
        for ($i = 0; $i < sizeof($data['employees']); $i++) {
            ...
            //print_r($this->db->last_query());die;
            $loan   = $this->admin_model->select_where('loan_applications', array(
                'employee_id' => $data['employees'][$i]['employee_id'],
                'is_approve' => 1
            ));
            $md     = explode(' ', $one);
            $md     = $md[0];
            $salary = $this->admin_model->select_where('salary', array(
                'employee_id' => $data['employees'][$i]['id'],
                'date' => $md
            ));
            if (!empty($salary)) {
                $ids[] = $i;
    ...
    {
        $subject = $this->admin_model->select_where('subject_case', array(
            'case_id' => $id
        ));
        $lead    = $_SESSION['id'];
        for ($i = 0; $i < sizeof($subject); $i++) {
    ...
    public function view_review($id, $eid)
    {
        ...
        $data['employee']         = $data['employee'][0];
        $data['employee_salary']  = $this->admin_model->select_where('employee_detail', array(
            'employee_code' => $data['employee']['employee_id']
        ));
        $data['salary_increment'] = $this->admin_model->select_where('salary_increment', array(
        ...
        if ($_SESSION['role'] == 'CM') {
            ...
            $data['review_detail'] = $this->admin_model->select_where('performance_reviews', array(
                'id' => $id,
        ...
        $data['form']          = 1;
        $data['less']          = $this->admin_model->select_where('review_less', array(
            'review_id' => $id
        ));
        $data['add']           = $this->admin_model->select_where('review_add', array(
            'review_id' => $id
        ));
        $data['improvement']   = $this->admin_model->select_where('review_improvement', array(
            'review_id' => $id
        ));
        $data['traning']       = $this->admin_model->select_where('review_training', array(
            'review_id' => $id
        ));
        $data['remarks']       = $this->admin_model->select_where('performance_reviews_remarks', array(
            'performance_id' => $id
        ));
        //$data['reviews'] = $this->admin_model->get_review($id);
        //print_r($this->db->last_query());
    ...
    {
        $id = $this->admin_model->update("performance_reviews", array(
            $column => $data
        ), array(
            'id' => $id
        ));
    ...
    public function year_view_review($id, $eid)
    {
        ...
        $this->db->select('r.*')->from('review_less r')->join('performance_reviews p', 'p.id = r.review_id')->where('p.created_at >= ', $start)->where('p.created_at <= ', $end)->where('p.employee_id', $eid);
        $data['less'] = $this->db->get()->result_array();
        $this->db->select('r.*')->from('review_add r')->join('performance_reviews p', 'p.id = r.review_id')->where('p.created_at >= ', $start)->where('p.created_at <= ', $end)->where('p.employee_id', $eid);
        $data['add'] = $this->db->get()->result_array();
        $this->db->select('r.*')->from('review_improvement r')->join('performance_reviews p', 'p.id = r.review_id')->where('p.created_at >= ', $start)->where('p.created_at <= ', $end)->where('p.employee_id', $eid);
        $data['improvement'] = $this->db->get()->result_array();
        $this->db->select('r.*')->from('review_training r')->join('performance_reviews p', 'p.id = r.review_id')->where('p.created_at >= ', $start)->where('p.created_at <= ', $end)->where('p.employee_id', $eid);
    ...
    {
        $data['vendor'] = $this->admin_model->select_where('employee_itgs', array(
            'id' => $id
        ));
        $this->load->view('admin2/header');
    ...
    public function update_vendor()
    {
        ...
        $id   = $data['id'];
        unset($data['id']);
        $this->admin_model->update('employee_itgs', $data, array(
            'id' => $id
        ));
    ...
    {
        $data['client'] = $this->admin_model->select_where('client', array(
            'client_id' => $id
        ));
        $data['title']  = "Client Edit";
        $this->db->select('*')->from('employee_itgs')->where('role', 'CM')->or_where('role', 'TL')->or_where('role', 'manager');
        $data['employees']              = $this->db->get()->result_array();
        $data['countries']              = $this->db->get('country')->result_array();
        $data['scopes']                 = $this->db->get('scope_of_work')->result_array();
        $data['services']               = $this->db->get('client_services')->result_array();
        $data['assign_client_services'] = $this->admin_model->select_where('assign_client_services', array(
            'client_id' => $id
        ));
        $this->load->view('admin2/header');
    ...
    public function client_update()
    {
        ...
        $this->admin_model->update("client", $data_client, array(
            'client_id' => $client_id
        ));
        $this->admin_model->delete_data('assign_client_services', array(
            'client_id' => $client_id
        ));
        //$client_id=$this->db->insert_id();
        // echo $client_id;
    ...
    {
        $data = $this->input->post();
        $old  = $this->admin_model->select_where('login', array(
            'login_id' => $_SESSION['login_id'],
            'pass' => $data['old']
        ));
        if (empty($old)) {
        ...
        if ($id) {
            $employee = $this->admin_model->select_where('login', array(
                'login_id' => $_SESSION['login_id']
            ));
    ...
    {
        $pass = $this->admin_model->select_where('change_password_request', array(
            'code' => $code,
            'status' => '0'
        ));
        if (!empty($pass)) {
            $this->admin_model->update_data('change_password_request', array(
                'status' => '1'
            ), array(
                'code' => $code,
                'status' => '0'
            ));
            $this->admin_model->update_data('login', array(
                'pass' => $pass[0]['new_password']
            ), array(
                'login_id' => $pass[0]['user_id']
            ));
            redirect('admin/admin_dashboard');
        } else {
    ...
    public function export_client_report($type)
    {
        ...
        $fields    = array(
            ...
            'Price',
            'Case Status',
            'Case Hold on Insufficency',
            'Insufficency Completed'
        );
    ...
    {
        $this->admin_model->update('assign_activity_to_user', array(
            'hold_status' => 1,
            'hold_date' => date('Y-m-d')
        ), array(
            'id' => $id
    ...
    {
        $data = $this->admin_model->select_where('employee_detail', array(
            'employee_code' => $code
        ));
        if (!empty($data)) {
    ...
    public function fund_invoice($id)
    {
        ...
        $data['internal'] = $this->admin_model->select_where_row('employee_itgs', array(
            'role' => 'Internal Auditor'
        ));
        $data['finance']  = $this->admin_model->select_where_row('employee_itgs', array(
    ...
    public function vendor_payment_invoice()
    {
        if ($this->input->post()) {
            ...
            $data['vendors'] = $this->admin_model->select_where('employee_itgs', array(
                'role' => 'vendor'
            ));
            $this->load->view('admin2/header', $data);
    ...
    public function view_case_detail($case_id, $id)
    {
        ...
        $data['id']             = $id;
        $this->db->select('e.employee_name')->from('case_fund_request c')->join('employee_itgs e', 'c.vendor_id = e.id')->where('c.case_id', $case_id);
    ...
    public function view_emp_details($id)
    {
        ...
        $employee_id                         = $data['employee']['employee_id'];
        $data['detail']                      = $this->admin_model->select_where_row('employee_detail', array(
        ...
        $data['salary']                      = $this->admin_model->select_where('salary', array(
            'employee_id' => $id
        ));
        $data['employee_educations']         = $this->admin_model->select_where('employee_educations', array(
            'employee_id' => $recruitment_id
        ));
        $data['employee_employment_history'] = $this->admin_model->select_where('employee_employment_history', array(
            'employee_id' => $recruitment_id
        ));
    ...
    {
        $this->db->select('a.*, s.subject_name,c.reference_code,cf.charges')->from('assign_vendor_request a')->join('subject_case s', 'a.subject_id = s.id')->join('case_request c', 'a.case_id = c.id')->join('case_fund_request cf', 'cf.case_id = a.case_id and cf.vendor_id = a.vendor_id', 'left')->where('a.id', $id);
    ...
    {
        $this->db->select('id,full_name as title, dob')->from('employee')->where('dob IS NOT NULL');
        $data['records'] = $this->db->get()->result_array();
        $this->db->select('id,employee_name as title, created_at as dob')->from('employee_itgs')->where('created_at IS NOT NULL')->where('role !=', 'vendor');
    ...
    public function view_ceo_leaves($id = null, $month = null)
    {
        ...
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($first))->where('l.start_date <=', DATE($last))->where('l.is_approve', '4')->group_by('e.id');
        $e = $this->db->get_compiled_select();
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($one))->where('l.start_date <=', DATE($two))->where("(l.is_approve = '4' OR l.is_approve = '7')")->group_by('e.id');
        $ec = $this->db->get_compiled_select();
        $this->db->select('e.id,e.employee_name,e.employee_id,sum(ed.sick_leave + ed.casual_leave + ed.annual_leave) as leaves,el.days as old_leaves,ec.days as month_leaves')->from('employee_itgs e')->join('employee_detail ed', 'ed.employee_code = e.employee_id', 'left')->join('(' . $e . ') el', 'e.employee_id = el.employee_id', 'left')->join('(' . $ec . ') ec', 'e.employee_id = ec.employee_id', 'left')->group_by('e.id');
    ...
    public function attendance()
    {
        ...
        $this->db->select('a.*,e.employee_name,e.designation,e.department')->from('attendance a')->join('employee_itgs e', 'e.employee_id = a.employee_id')->where('a.date >=', $one)->where('a.date <=', $two)->where('a.employee_id !=', '')->group_by('a.id');
        $data['attendance'] = $this->db->get()->result_array();
        $data['current']    = $one;
        $data['last']       = $two;
        
        $total_days = $this->countDays(date('Y', strtotime($one)), date('m', strtotime($one)), array(
            0,
            6
        ));
        $this->db->select("id as total,employee_id,clock_in,STR_TO_DATE('091559','%h%i%s') as last,'12:00:01' as end")->from('attendance')->where('date >=', $one)->where('date <=', $two)->where('employee_id !=', '')->having('clock_in > last AND clock_in < end');
        $c = $this->db->get_compiled_select();
        $this->db->select("id as total,employee_id,clock_in,'12:00:00' as last")->from('attendance')->where('date >=', $one)->where('date <=', $two)->where('employee_id !=', '')->having('clock_in > last');
        $h = $this->db->get_compiled_select();
        $this->db->select("id as total,employee_id,clock_out,'03:00:01' as last")->from('attendance')->where('date >=', $one)->where('date <=', $two)->where('employee_id !=', '')->having('clock_out < last');
        $s = $this->db->get_compiled_select();
        $this->db->select("count(id) as total, employee_id")->from('attendance')->group_by('employee_id')->where('date >=', $one)->where('date <=', $two)->where('employee_id !=', '');
        $a = $this->db->get_compiled_select();
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($one))->where('l.start_date <=', DATE($two))->where("(l.is_approve = '4' OR l.is_approve = '7')")->where('l.nature_of_leave', 'Half Day')->group_by('e.id');
        $e = $this->db->get_compiled_select();
        $this->db->select('sum(l.total_days) as days, e.employee_id')->from('employee_itgs e')->join('leave_applications l', 'l.employee_id = e.employee_id', 'left')->where('l.start_date >=', DATE($one))->where('l.start_date <=', DATE($two))->where("(l.is_approve = '4' OR l.is_approve = '7')")->where('l.nature_of_leave !=', 'Half Day')->group_by('e.id');
        $ec = $this->db->get_compiled_select();
        $this->db->select('e.employee_name, concat(' . $total_days . ' - a.total) as leaves, count(c.total) as late, group_concat(DISTINCT(h.total) separator ",") as hafe, group_concat(DISTINCT(s.total) separator ",") as short, ec.days as approved, eh.days as short_approved')->from('employee_itgs e')->join('(' . $a . ') a', 'a.employee_id = e.employee_id')->join('(' . $c . ') c', 'e.employee_id = c.employee_id')->join('(' . $h . ') h', 'e.employee_id = h.employee_id', 'left')->join('(' . $s . ') s', 'e.employee_id = s.employee_id', 'left')->join('(' . $e . ') eh', 'e.employee_id = eh.employee_id', 'left')->join('(' . $ec . ') ec', 'e.employee_id = ec.employee_id', 'left')->group_by('e.id')->order_by('e.employee_name');
        $data['employees'] = $this->db->get()->result_array();
        //print_r($this->db->last_query());
        //echo '<pre>';print_r($data);die;
        $this->load->view('admin2/header', $data);
    ...
    public function attendance_upload()
    {
        ...
        foreach ($cells as $c) {
            ...
            $data        = $this->admin_model->select_where_row('attendance', array(
                'employee_id' => $employee_id,
                'date' => $date
            ));
            if (!$data) {
                ...
                $this->admin_model->update('attendance', array(
                    'clock_out' => $time
                ), array(
                    'id' => $data['id']
                ));
    ...
    {
        $this->db->select('f.name, group_concat(fa.title separator ":") as titles, group_concat(fa.attechment separator ":") as attechments')->from('forms f')->join('forms_attechments fa', 'fa.forms_id = f.id')->group_by('f.id');
    ...
    public function insert_forms()
    {
        ...
        $id       = $this->admin_model->insert('forms', array(
            'name' => $name
        ));
        //$froms = [];
    ...
    {
        $data = $this->admin_model->select_where('sub_scope', array(
            'scope_id' => $scope_id
        ));
        echo json_encode($data);
    }
