...
$vendor_total=$this->db->query("select SUM(charges) as charges, em.vendor_type from case_fund_request join employee_itgs em on em.id = case_fund_request.vendor_id where  is_approve=1 and activity_id=".$case['activity_id'].' and subject_id='.$case['subject_id'])->row_array();
...
$total=$funds_total['of']+$funds_total['vc']+$funds_total['epc']+$funds_total['mcc']+$funds_total['bc']+$funds_total['pcr']+$funds_total['otc']+$vendor_charges;
// if ($case['client_type']=="INT") {
//   $conversion_rate = ($activity_price['conversion_rate']) ? $activity_price['conversion_rate'] : $rate;
//   $total = round($total*$conversion_rate);
// }
?>
...
<td><span class="footable-toggle"><?php echo $total; ?> PKR</span></td>
...
if ($case['client_type']=="INT") {
  $conversion_rate = ($activity_price['conversion_rate']) ? $activity_price['conversion_rate'] : $rate;
  $assigne_price = round($activity_price['activity_price']*$conversion_rate);
}
?>
