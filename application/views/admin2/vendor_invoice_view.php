...
h4.mar {
  ...
  <!-- /.Navbar  Static Side -->
		...
		<table id="ex" class="table table-bordered table-striped table-hover" cellspacing="0" width="100%">
         ...
         <th>Vendor Charges (Excluding Sales Tax)</th>
          <th>Sales Tax</th>
          <th>Charges (Including Sales Tax)</th>
          <th>Paid?</th>
          <th>Action?</th>
...
<tr>
  ...
  <td><?php echo $row['vendor_charges'] ?></td>
  <td><?php echo $row['vendor_charges']/100*13; ?></td>
  <td><?php echo $row['vendor_charges']+$row['vendor_charges']/100*13; ?></td>
  <td><?php echo $row['status']==0? "Unpaid" :"Paid";  ?></td>
  <td>
    <a href="<?php echo base_url() ?>admin/update_vendor_invoice_status?client=<?php echo $row['client_id'] ?>&caseID=<?php echo $row['case_id'] ?>" class="sweets-alert" target="_blank" ><i class="fa fa-dollar"></i></a>
